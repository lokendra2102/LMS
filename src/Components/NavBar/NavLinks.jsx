...
function NavLinks({updateMembership, user, bought}) {
    //Premium Modal
    ...
    return (
        <Navbar collapseOnSelect expand="lg" bg="light" variant="light">
            <Container fluid>
                ...
                <Navbar.Collapse id="responsive-navbar-nav" className=''>
                    <Nav className="me-auto ms-auto">
                        ...
                        { user && bought.size > 0 ? <NavLink className='text-uppercase home_nav px-3 nav-link' to="/user/my-courses">My Courses</NavLink> : <></>}
                    </Nav>
                    {!user ? <></> :
                        user && user.isPremium ? <></> :
            ...
            <BuyPremiumModal show={show} handleClose={handleClose} user={user} updateMembership={updateMembership} />
        </Navbar>
    )
}
