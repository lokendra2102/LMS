...
function HomeCardHeader({paths, isAbout}) {
  return (
    <Container fluid className={`px-5 ps-4 ${!isAbout ? 'd-flex align-items-center justify-content-between book_head' : 'text-center aboutHead px-0 py-4 text-light'}`}>
        <h1 className={`fs-3 fw-bold text-center text-uppercase book_headline ${isAbout ? 'text-decoration-underline' : ''}`}>{paths}</h1>
        {
          isAbout ? 
          <p className='mb-0 mx-lg-5 mx-xl-5 mx-md-3 mx-sm-1 text-center fs-5 pt-4'>
